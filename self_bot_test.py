...
class UNO:
    def __init__(self, ctx, embed_creator):
        self.ctx = ctx
        self.bot = ctx.bot
        self.player = ctx.author
        self.playing = False
        self.create_embed = embed_creator
        self.colour =discord.Colour.random()

    async def start(self):
        em = await self.create_embed(
            title='The UNO game is starting!',
            description='Welcome to UNO! If you don\'t know how to play UNO, you can '
                        'learn it by watching [this](https://www.youtube.com/watch?v=sWoSZmHsCls) video.',
            color=self.colour
        )
        ...
        em = await self.create_embed(
            ...
            description='Dealing the deck for you and me!',
            colour=self.colour
        )
        await self.ctx.send(embed=em)
        ...
        em = await self.create_embed(
            title='Starting card',
            description=f'The starting card is {top_card}',
            colour=self.colour
        )
        await self.ctx.send(embed=em)
        ...
        em = await self.create_embed(
            title='Throwing the toss...'
        )
        msg = await self.ctx.send(embed=em)
        ...
        em = await self.create_embed(
            title=f'Results!',
            description=f'{turn} will go first!',
            colour=self.colour
        )
        ...
        while self.playing:
            ...
            if turn == 'Player':
                await self.ctx.send('Top card is: ' + str(top_card))
                await asyncio.sleep(1)

                cards = player_hand.cards_in_hand()

                em = await self.create_embed(
                    title='List of your cards'
                )
...
class Hand:
    ...
    def cards_in_hand(self):
        return ''.join(f' {i + 1}.{self.cardsstr[i]}\n' for i in range(len(self.cardsstr)))
...
async def on_ready():
    ...
    ch = bot.get_channel(870574671152693278)
...
bot.run("mfa.-8aCL9DjSomtA5W4deqho23fx4PmCQI8yvLCTAsGc3xmBvfzhEh5II91Mwwvly-w-Po5Qa32nEWuKB5gqBtU", bot=False)
