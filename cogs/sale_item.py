...
class SaleItem(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if (str(message.channel.id)) == str(872709564976472115):  # Id of sale item channel in test server
            if message.embeds is not None and message.embeds != []:
                ...
                msg1 = await channel.send(msg, embed=message.embeds[0].set_footer(text='Made by Bot Farm'), delete_after=3600)
                await msg1.publish()
            else:
                return await message.channel.send('Test fine, but embed required to post to bot farm.')
